...
CREATE PROCEDURE [dbo].[uspPULSEApiGetPulseScoreForStudent_temp]
...
Execute Sample	: 
	...
	EXEC dbo.uspPULSEApiGetPulseScoreForStudent_temp
		@pSourceSystemID			= 2
		, @pStudentID				= 3122032 --NULL --76662 --78456 --139820 --176262 --76726
		, @pEndDate					= '2011-05-18' --NULL --'2017-03-09' --'2016-12-09' --'2017-07-19' --'2017-04-20' --'1996-03-17' --'2018-07-17' --'2018-07-18'
		, @pReleaseDate					= '09/07/2018' --NULL --'2017-03-09' --'2016-12-09'
...
**********************************************************************************************************************************/
	...
	SELECT
		...
		, CASE
		  ...
		  ,DC.KeyClassID as KeyID
		, CASE
			WHEN(isNull(DC.GraduationDate,1900-01-01) <= @ReleaseDate ) THEN 'PULSE 1.0'
			ELSE 'PULSE 2.0'
		  END AS pulseAlgo
