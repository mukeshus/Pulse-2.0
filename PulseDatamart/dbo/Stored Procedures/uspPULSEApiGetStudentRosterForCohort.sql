...
CREATE PROCEDURE [dbo].[uspPULSEApiGetStudentRosterForCohort]
...
Execute Sample	: 
	DECLARE @pClassID				BIGINT	= 48962	--NULL	--32513	--12815
	DECLARE @pClassName				NVARCHAR(128)
	EXEC dbo.uspPULSEApiGetStudentRosterForCohort
		@pSourceSystemID			= 2
...
**********************************************************************************************************************************/
	...
	WHERE
		...
		AND ISNULL(CONVERT(DATE,FromDate),'1999-01-01') <= @EndDate
		AND (ToDate IS NULL OR CONVERT(DATE,ToDate) >= @EndDate)
